...
def precision(rank, ground_truth):
    ...
    #result=[top1_pre top2_pre topK_pre]
    return result
...
def ndcg(rank, ground_truth):
    ...
    # calculate idcg
    idcg = np.cumsum(1.0 / np.log2(np.arange(2, len_rank + 2))) #若该用户测试集物品数<|topK|==>|topN|
    #/sum_{i=1}{k}(1/log(1+i))
...
def eval_score_matrix_foldout(score_matrix, test_items, top_k=50, thread_num=None):
    ...
    with ThreadPoolExecutor(max_workers=thread_num) as executor:
        ...
        #len(test_items) test中user数量

    result = list(batch_result)  # generator to list
    return np.array(result)  # list to ndarray


def load_eval_score_matrix_foldout(score_matrix, test_items, top_k=50, thread_num=None):
    def _eval_one_user(idx):
        scores = score_matrix[idx]  # all scores of the test user
        test_item = test_items[idx]  # all test items of the test user
        ranking = argmax_top_k(scores, top_k)  # Top-K items
    ...
    for i in range(len(test_items)):
        batch_result, ranking = _eval_one_user(i)
        batch_results.append(batch_result)
        rankings.append(ranking)

    result = list(batch_results)  # generator to list
    ranking = list(rankings)
    return np.array(result) ,np.array(ranking) # list to ndarray